var Le={getAllDepartments:"http://localhost:5000/api/departments/",getDepartment:a=>`http://localhost:5000/api/departments/name/${a}`,updateEmployee:a=>"http://localhost:5000/api/departments/employee/update",deleteEmployee:a=>`http://localhost:5000/api/departments/employee/id/${a}`},L=Le;var Se=async a=>await(await fetch(a)).json(),Ce=async a=>await(await fetch(a,{method:"DELETE",redirect:"follow"})).json(),De=async(a,n)=>{let s=new Headers;s.append("Content-Type","application/json");let o=JSON.stringify(n),m=await(await fetch(a,{method:"PUT",headers:s,body:o,redirect:"follow"})).text();console.log(m)},Ae={getRequest:Se,deleteRequest:Ce,putRequest:De},S=Ae;var Oe=function(){let a=null;function n(){let s=document.createElement("img");document.body.appendChild(s);let o=async m=>{let u=await S.deleteRequest(L.deleteEmployee(m.detail.id));console.log(u),s.dispatchEvent(new CustomEvent("removeItem",{bubbles:!0,detail:u}))},c=async m=>{let u=await S.putRequest(L.updateEmployee(m.detail.payload.id),m.detail.payload)},h=async m=>{let u=await S.getRequest(L.getDepartment(m.detail.name));s.dispatchEvent(new CustomEvent("updateDisplay",{bubbles:!0,detail:u}))};return s.addEventListener("deleteEmployee",o),s.addEventListener("editEmployee",c),s.addEventListener("sideBarSelect",h),{handle:s}}return{getInstance:function(){return a||(a=n()),a}}}(),D=Oe;var G=`
...
`,mt={parentTemplate:ut,childTemplate:ct},H=mt;var dt=a=>{let n=document.querySelector("#managers"),s=a.filter(c=>c.role),o=T(H.parentTemplate);return s.forEach(c=>{let h=A(H.childTemplate,c),m=T(h);o.appendChild(m)}),o},me=dt;var de=`
